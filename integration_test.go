...
func TestIntegrationAgentCompletionAndQueueProcessing(t *testing.T) {
	...
	testManager.SetAgentStartCallback(func(agentID, folder, prompt, queueID string) {
		...
		RegisterAgentForUser(agentID, 12345) // Register for notifications
	})
	...
	RegisterAgentForUser(agent1ID, 12345)
	...
	recordEvent("Starting agent monitor")
...
func TestZombieProcessDetection(t *testing.T) {
	...
	mockAgent := &ZombieAgent{
		...
		Status:       codeagent.StatusRunning, // Agent thinks it's running
	...
	if mockAgent.Status == codeagent.StatusRunning && mockAgent.ProcessState == "finished" {
		log.Printf("[ZOMBIE-TEST] Detected zombie process - should update status to Finished")
		mockAgent.Status = codeagent.StatusFinished
