	...
	return h.Div(h.ID("git-section"), h.Class("section"),
		...
		h.Div(h.Class("git-pr-section"),
			...
			h.Div(h.Class("pr-operations"),
				// PR Creation Form
				h.Div(h.Class("pr-create-form"),
					h.H4(g.Text("Create Pull Request")),
					h.Form(
						h.ID("pr-create-form"),
						h.Method("post"),
						h.Action("/api/git/pr/create"),
						
						h.Input(h.Type("hidden"), h.Name("folder"), h.Value(folderPath)),
						
						h.Div(h.Class("form-group"),
							h.Label(h.For("pr-branch"), g.Text("Branch:")),
							h.Input(
								h.Type("text"),
								h.ID("pr-branch"),
								h.Name("branch"),
								h.Placeholder("feature/my-branch"),
								h.Class("form-control"),
								h.Required(),
							),
						),
						...
						h.Div(h.Class("form-group"),
							h.Label(h.For("pr-title"), g.Text("Title:")),
							h.Input(
								h.Type("text"),
								h.ID("pr-title"),
								h.Name("title"),
								h.Placeholder("Add new feature"),
								h.Class("form-control"),
								h.Required(),
							),
						),
						...
						h.Div(h.Class("form-group"),
							h.Label(h.For("pr-body"), g.Text("Description:")),
							h.Textarea(
								h.ID("pr-body"),
								h.Name("body"),
								h.Placeholder("Describe your changes..."),
								h.Class("form-control"),
								h.Rows("5"),
							),
						),
						...
						h.Div(h.Class("form-group"),
							h.Label(h.For("pr-base"), g.Text("Base Branch:")),
							h.Input(
								h.Type("text"),
								h.ID("pr-base"),
								h.Name("base"),
								h.Placeholder("main (default)"),
								h.Class("form-control"),
							),
						),
